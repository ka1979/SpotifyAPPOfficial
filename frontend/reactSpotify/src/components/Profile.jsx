...
export default function Profile() {
  ...
  const getProfile = async () => {
    try {
      const response = await axios.post(
        `http://localhost:3000/yash/profile`,
        { email: email }
      );
      setName(response.data.result.name);
      setIsPublic(response.data.result.isPublic);
...
<>
    ...
    <div className='view-container'>
      ...
      <div className='cards-container'>
        <div className="top-card">
          <h2>Display Name:</h2>
          <div className="shadow-container" style={{ marginTop: '10px', padding: '5px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <h3>{name}</h3>
          </div>
        </div>
        ...
        <div className="top-card">
          {isPublic ?
            <h2><BsFillUnlockFill /> Public Profile</h2> :
            <h2><BsFillLockFill /> Private Profile</h2>
          }
          <Switch
            checked={isPublic}
