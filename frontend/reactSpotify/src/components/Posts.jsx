...
const Posts = () => {
  ...
  const handleCreatePost = async () => {
    try {
      await axios.post('http://localhost:3000/posts', { forumId: id, title: newPostTitle, creator: 'Your name here' });
      setNewPostTitle("");
      handleClose();
      fetchPosts();
    } catch (error) {
  ...
  const handleLike = async (postId) => {
    try {
      await axios.post(`http://localhost:3000/posts/${id}/like`, { forumId: id, postId });
      fetchPosts();
    } catch (error) {
